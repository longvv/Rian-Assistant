...
|-------|----------|----------|
| AI API costs spiral out of control | high | **Controlling AI Costs**: Hard limits on usage per user, caching, batching, cheaper models for simple tasks. |
| App breaks when hitting API rate limits | high | **Handling Rate Limits**: Implement robust retry logic with exponential backoff. Add a queue system for large bursts. |
| AI gives wrong or made-up information | high | **Handling Hallucinations**: Constrain prompts, require citations from given context, validate structured outputs. |
| AI responses too slow for good UX | medium | **Improving AI Latency**: Switch from GPT-4 to Claude 3 Haiku or GPT-4o-mini, stream tokens, pre-compute responses. |
